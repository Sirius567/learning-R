...
# write.csv for matrices and data.frames

# find in R help how to write and read csv files to local disk
# create a specific folder in your current working directory for this task

# create a 10000 x 10 matrix with random numbers, and save it writing a csv 

m<-matrix(rnorm(100000),nrow=10000)
colnames(m)<-paste0(rep('col',ncol(m)),seq(1,ncol(m)))
rownames(m)<-paste0(rep('row',nrow(m)),seq(1,nrow(m)))
...
# first create a subset of the first 10 rows of the dataset, and save it as a csv

write.table(airquality[1:10,],'example_data.frame.csv',row.names=FALSE, na="NA",quote= FALSE, sep=",")

# then find how to append new data to the previous saved dataset, and append the next 10 rows
# from the original data

write.table(airquality[11:20,],'example_data.frame.csv', row.names=FALSE, col.names=FALSE,na="NA",
            append = TRUE,quote= FALSE, sep=",")

# read the csv you just wrote and save if to a different object
